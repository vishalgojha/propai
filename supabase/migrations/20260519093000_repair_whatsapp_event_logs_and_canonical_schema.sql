...
create index if not exists idx_canonical_records_kind_seen
    on public.canonical_records (record_kind, last_seen_at desc);

create index if not exists idx_canonical_records_location
    on public.canonical_records (locality, city, deal_type, asset_class);

create table if not exists public.canonical_record_evidence (
    id uuid primary key default gen_random_uuid(),
...
create index if not exists idx_canonical_record_evidence_canonical
    on public.canonical_record_evidence (canonical_record_id, created_at desc);

create index if not exists idx_canonical_record_evidence_tenant
    on public.canonical_record_evidence (tenant_id, created_at desc);
...
create unique index if not exists idx_source_reliability_tenant_phone
    on public.source_reliability (tenant_id, source_phone);
...
alter table public.stream_items
    ...
    add column if not exists canonical_record_id uuid references public.canonical_records(id) on delete set null,
...
create index if not exists idx_stream_items_canonical_record
    on public.stream_items (canonical_record_id, created_at desc);

create index if not exists idx_stream_items_semantic_fingerprint
    on public.stream_items (semantic_fingerprint_text);
...
begin
    if exists (
        ...
        where table_schema = 'public'
          and table_name = 'whatsapp_event_logs'
          and column_name = 'session_id'
    ) then
        execute $sql$
            update public.whatsapp_event_logs
            set session_label = coalesce(session_label, session_id)
            where coalesce(session_label, '') = ''
              and coalesce(session_id, '') <> ''
        $sql$;
    end if;
end $$;

create index if not exists whatsapp_event_logs_session_label_created_idx
    on public.whatsapp_event_logs (session_label, created_at desc);

alter table public.canonical_records enable row level security;
alter table public.canonical_record_evidence enable row level security;
alter table public.source_reliability enable row level security;

drop policy if exists canonical_records_select_authenticated on public.canonical_records;
create policy canonical_records_select_authenticated
    on public.canonical_records
    for select
    to authenticated
    using (true);

drop policy if exists canonical_record_evidence_select_owner on public.canonical_record_evidence;
create policy canonical_record_evidence_select_owner
    on public.canonical_record_evidence
    for select
    to authenticated
    using (tenant_id = auth.uid());

drop policy if exists source_reliability_select_owner on public.source_reliability;
