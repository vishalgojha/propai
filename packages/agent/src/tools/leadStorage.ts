...
export const LeadStorageTools = {
	store_leads: {
		description: 'Use this only when the lead has already been validated and the broker or supervisor has confirmed persistence. It should fire for final save actions, not for extraction or scoring, and it keeps writes idempotent so the same broker message is not stored twice.',
		schema: z.object({
