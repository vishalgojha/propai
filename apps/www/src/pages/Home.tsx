...
export default function Home() {
  ...
  return (
    <div className="space-y-24 pb-24">
      <Helmet>
        <title>PropAI Pulse | Real-Time Off-Market Mumbai Real Estate</title>
        <meta name="description" content="Access Mumbai's off-market property inventory before it hits MagicBricks or 99acres. Sourced directly from real-time broker broadcasts using AI." />
        <meta property="og:title" content="PropAI Pulse | Off-Market Mumbai Real Estate Intelligence" />
        <meta property="og:description" content="Capture off-market rentals, homes, and offices directly from broker networks the moment they are sent." />
      ...
      <section className="mx-auto max-w-7xl px-5">
        <div className="flex items-end justify-between mb-10">
          ...
          <Link to="/listings" className="text-[12px] font-bold text-[var(--accent)] hover:underline flex items-center gap-1 group">
      ...
      <section className="mx-auto max-w-7xl px-5 relative py-20 overflow-hidden">
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-5 max-w-6xl mx-auto relative z-10">
          {[
            ...
            <Link 
              key={loc.n} 
              to={`/listings?locality=${encodeURIComponent(loc.n)}`}
              className={cn(
      ...
      <section className="mx-auto max-w-5xl px-5 shadow-[0_24px_80px_rgba(62,232,138,0.06)]">
        <div className="rounded-[24px] bg-gradient-to-br from-[var(--bg-elevated)] to-[#0c1a12] border border-[color:var(--accent-border)] p-12 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link to="/listings" className="inline-flex items-center justify-center gap-2 rounded-[12px] bg-[var(--accent)] px-8 py-4 text-[13px] font-bold uppercase tracking-wider text-[var(--on-propai-green)] shadow-xl hover:brightness-110 transition-all">
              Browse Listings
            </Link>
            <Link to="/broker/signup" className="inline-flex items-center justify-center gap-2 rounded-[12px] border border-[color:var(--border)] bg-[var(--bg-surface)] px-8 py-4 text-[13px] font-bold uppercase tracking-wider text-[var(--text-primary)] hover:bg-[var(--bg-hover)] transition-all">
              For Brokers
            </Link>
          </div>
        </div>
