  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-12 space-y-12">
      <div className="space-y-6">
        <Link to="/" className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.1em] text-[var(--text-muted)] hover:text-[var(--accent)] transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {listings.length > 0 ? (
          ...
          <div className="col-span-full py-32 text-center rounded-[20px] border border-dashed border-[color:var(--border)] bg-[var(--bg-elevated)]/30">
            <p className="text-[var(--text-secondary)]">No active inventory in {localityName} right now. <Link to="/listings" className="text-[var(--accent)] font-bold">Browse all listings</Link></p>
          </div>
        )}
      </div>
    </div>
