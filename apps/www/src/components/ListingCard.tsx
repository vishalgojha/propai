...
export default function ListingCard({ listing, compact }: ListingCardProps) {
  ...
  if (compact) {
    return (
      ...
      >
        <div className="shrink-0 w-16 h-16 rounded-lg bg-slate-200 dark:bg-[var(--bg-elevated)] flex items-center justify-center overflow-hidden">
          {coverImage ? (
            <img src={coverImage} alt="" className="w-full h-full object-cover" />
          ) : (
            <Home className="h-6 w-6 text-slate-500 dark:text-[var(--text-muted)]" />
          )}
        </div>
    ...
    <article className="group flex h-full animate-stream-in flex-col overflow-hidden rounded-[18px] border border-[color:var(--border-strong)] bg-[var(--bg-surface)] shadow-[0_14px_36px_rgba(0,0,0,0.16)] transition-all duration-300 hover:-translate-y-1 hover:border-[color:var(--accent-border)] hover:shadow-[0_22px_46px_rgba(0,0,0,0.22)]">
      <Link href={detailsHref} className="relative block aspect-[4/3] overflow-hidden bg-[var(--bg-elevated)]">
        {coverImage ? (
          ...
          <div className="flex h-full items-center justify-center bg-slate-200 dark:bg-[linear-gradient(135deg,rgba(62,232,138,0.10),rgba(59,130,246,0.08)_48%,rgba(255,255,255,0.03))]">
            <div className="flex h-14 w-14 items-center justify-center rounded-[18px] bg-slate-300 dark:bg-[var(--bg-surface)]/75 text-slate-600 dark:text-[var(--accent)] shadow-[0_18px_40px_rgba(0,0,0,0.18)]">
