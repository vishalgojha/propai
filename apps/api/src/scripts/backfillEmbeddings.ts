...
async function backfillTable(table: string) {
  ...
  while (true) {
    const { data: rows, error } = await supabase
      .from(table)
      .select("id, locality, bhk, price_numeric, price_label, type, furnishing, raw_text, building_name, property_use, city, property_category, deal_type, asset_class, micro_location")
      .is("embedding", null)
      .order("id")
    ...
    for (const row of rows) {
      ...
      if (!fingerprint) { failed++; continue; }
