...
function listResourceUris(items: Array<{ remote_jid: string }>) {
...
export function registerMcpResources(server: McpServer, context: ToolContext = {}) {
  ...
  server.registerResource(
    "broker-thread-template",
    new ResourceTemplate("propai://thread/{remote_jid}", {
      list: async () => {
        const id = requireBrokerId(context);
