...
import { ZapIcon, ListingIcon, RequirementIcon, MessageCircleIcon } from '../lib/icons';
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 md:p-6">
      ...
      <div className="flex gap-1 rounded-[12px] border border-[color:var(--border)] bg-[var(--bg-elevated)] p-1">
        ...
        >
          ...
          {listings.length > 0 && (
            <span className={`ml-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${
              tab === 'listings' ? 'bg-black/10' : 'bg-[var(--bg-hover)]'
            }`}>
              {listings.length}
            </span>
          )}
        ...
        >
          ...
          {requirements.length > 0 && (
            <span className={`ml-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${
              tab === 'requirements' ? 'bg-black/10' : 'bg-[var(--bg-hover)]'
            }`}>
              {requirements.length}
            </span>
          )}
      ...
      {loading ? (
        ...
        <div className={`${panelClass} text-center py-16`}>
        ...
        <div className="flex justify-center pt-2">
          <button
            ...
            onClick={() =>
              askPulse(
                tab === 'listings'
                  ? `Analyse my ${listings.length} saved listings — which ones need attention?`
                  : `Match my ${requirements.length} buyer requirements against available inventory`,
              )
            }
          >
