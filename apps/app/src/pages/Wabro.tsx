...
type AccessState = {
  hasAccess: boolean;
  reason: 'owner' | 'workspace-plan' | 'upgrade';
};
...
function resolveWabroAccess(user: ReturnType<typeof useAuth>['user']): AccessState {
  const normalizedPlan = String(user?.subscription?.plan || '').trim().toLowerCase();
  const normalizedStatus = String(user?.subscription?.status || '').trim().toLowerCase();
  const isOwner = user?.appRole === 'super_admin';
  const isWorkspacePaid = ['solo', 'pro', 'team'].includes(normalizedPlan) && ['active', 'trial', 'trialing'].includes(normalizedStatus || 'trial');
  ...
  if (isWorkspacePaid) {
    return { hasAccess: true, reason: 'workspace-plan' };
  }

  return { hasAccess: false, reason: 'upgrade' };
}
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 max-w-xl text-[13px] leading-6 text-[var(--text-secondary)]">
            {body || 'WaBro is a separate broadcast product for broker campaigns. This route is ready inside PropAI, but access should be granted through the WaBro yearly plan.'}
          </p>
        </div>
  ...
  return (
    ...
    >
      <AccessGate
        ...
        body="You can review the setup flow before the plan is active, but Android execution and campaign launch belong to the WaBro paid product."
      />
