...
import { SmartphoneIcon, SearchIcon, RefreshIcon, LoaderIcon, EyeIcon } from '../lib/icons';
...
export const ClientContacts: React.FC = () => {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const resp = await backendApi.get(ENDPOINTS.dmContacts.list, { params: { label: 'client' } });
  ...
  return (
    <div className="rounded-[28px] border border-[#202c33] bg-[#111b21] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-white">Client Contacts</h2>
          <p className="text-sm text-[#8696a0]">DM contacts tagged as client — private, never parsed</p>
        </div>
        <button
          type="button"
          onClick={() => void load()}
      ...
      <p className="mb-4 rounded-lg bg-[#1a2e3a] px-4 py-2 text-xs text-[#8696a0]">
        <EyeIcon className="mr-1 inline-block h-3 w-3" />
        Client DMs are never parsed by the AI. These contacts are stored for your reference only.
      </p>

      <div className="space-y-2">
        {filtered.map(c => (
          <div key={c.remote_jid} className="flex items-center gap-3 rounded-lg bg-[#1f2c33] px-4 py-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#233138] text-[#53bdeb]">
        ...
        {!loading && filtered.length === 0 && (
          <p className="py-8 text-center text-sm text-[#8696a0]">No client contacts yet. Tag a DM as Client from the Inbox.</p>
        )}
      </div>
    </div>
  );
