...
export default function Onboarding() {
    ...
    const handleSend = async () => {
        ...
        if (step === 'PHONE') {
            setStep('CODE');
            setTimeout(async () => {
                const teamParts = userText.split('|').map((part) => part.trim()).filter(Boolean);
                const phoneNumber = addingTeamMember ? teamParts[0] || userText : userText;
                const ownerName = addingTeamMember ? teamParts[1] || 'Team member' : undefined;
                const label = addingTeamMember
                    ? ownerName || `Team ${phoneNumber.replace(/\D/g, '').slice(-4) || 'member'}`
                    : 'Owner';

                await apiFetch('/api/whatsapp/connect', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ tenantId: user.id, phoneNumber, label, ownerName }),
                });
                ...
                const poll = setInterval(async () => {
                    ...
                    if (data.qr && data.qr.length === 8) {
                        ...
                        startConnectionPolling(addingTeamMember ? 'team' : 'owner');
                    }
                }, 2000);
            }, 800);
            return;
    ...
    const startConnectionPolling = (mode: 'owner' | 'team' = 'owner') => {
        const poll = setInterval(async () => {
            const supabase = getSupabaseClient();
            if (!supabase || !user) {
                clearInterval(poll);
            ...
            if (data.status === 'connected') {
                ...
                if (mode === 'team') {
                    ...
                    setStep('TEAM');
                    addMessage('ai', 'Team member connected. Add another teammate?');
